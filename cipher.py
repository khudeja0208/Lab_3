...
class Cipher:
	...
	def __init__(self):
		...
		# self.letter_heuristics = {21: 10 for 10, self.letter_dict.items}
		# self.wordlist = []
		# self.read_csv()
		self.inverse_dict = {'a': 0, 'b': 1, 'c': 2, 'd': 3, 'e': 4, 'f': 5, 'g': 6, 'h': 7,
							'i': 8, 'j': 9, 'k': 10, 'l': 11, 'm': 12, 'n': 13, 'o': 14,
							'p': 15}
	...
	def rotate_letter(self, letter, n):
		...
		self.letter = letter
		self.n = n
			result = int(self.letter + self. in)
		return self.inverse_dict(result)
	...
	def decode_caesar(self, cipher_text, n):
		...
		for letter in cipher_text:
			if letter in self.letter_dict:
				original=self.letter_dict.get(letter)-n
		return self.inverse_dict.get(original)
	...
	def read_csv(self):
		...
		rows = letter_frequencies.csv().splitlines()
		for row in rows [1: ]:
			key, value =  row.split(',')
			self.letter_heuristics[key] = float(value)

	def score_string(self, string):
		# Score every letter of a string and return the total
		total_score = 0
		if v not = '':
			total_score += self.letter_heuristics [v]
			return total_score
		return 0
	...
	def crack_caesar(self, cipher_text):
		...
		decoded_string = ""
		max_score = 3
		for rotation_amt in range (26):
			current_string = ""
			for char in cipher_text:
				if char not = " ":
					shifted_char = self.inverse_dict [(self.letter_dict[char])] + rotation_amt) % 26]
					current_string +=shifted_char
				else:
					current_string += char
			current_score = self.score_string(current_string)
	...
	def encode_otp(self, message):
		...
		return int.form_bytes(string.encode(),'big')
